...
function AboutMe({ user, currentUser }) {
  ...
  function bioSubmit() {
    return axios.put(`http://ec2-3-16-44-36.us-east-2.compute.amazonaws.com:8080/userprofile/${user.username}`, { user: user.username, bio: bio })
      .then(() => {
        user.aboutMe = bio;
        setEditing(false);
      })
  ...
  function handleFave() {
    if (currentUser) {
      return axios.put('http://ec2-3-16-44-36.us-east-2.compute.amazonaws.com:8080/userprofile', { currentUser: currentUser, user: user.username })
      .then((data) => {
        setFaved(true);
      })
    }
  ...
  function handleUnfave() {
    if (currentUser) {
      return axios.put('http://ec2-3-16-44-36.us-east-2.compute.amazonaws.com:8080/userprofilex', { currentUser: currentUser, user: user.username})
        .then((data) => {
          setFaved(false);
        })
    }
