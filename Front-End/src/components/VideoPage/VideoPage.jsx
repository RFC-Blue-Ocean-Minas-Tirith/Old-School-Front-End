...
function VideoPage() {
  ...
  const updateVote = (e) => {
    ...
    if (currentVid.votes[button].usernames.indexOf(currUser.username) === -1) {
      ...
      setCurrentVid(vid);
      axios.put('http://ec2-18-220-24-246.us-east-2.compute.amazonaws.com/video/vote', { videoID: currentVid._id, vote: button, username: currUser.username })
        .catch((err) => {
  ...
  const favorite = (e) => {
    ...
    if (favorited[0] === 'unfavorited') {
      ...
      axios.put('http://ec2-18-220-24-246.us-east-2.compute.amazonaws.com/userprofile', { currentUser: currUser, user: currentVid.username })
        .catch((err) => {
          console.log(err);
        });
    } else {
      setFavorited(['unfavorited', 'Favorite this Creator!']);
      axios.put('http://ec2-18-220-24-246.us-east-2.compute.amazonaws.com/userprofilex', { currentUser: currUser, user: currentVid.username })
        .catch((err) => {
          console.log(err);
        });
    }
  ...
  const report = (e) => {
    ...
    axios.put('http://ec2-18-220-24-246.us-east-2.compute.amazonaws.com/video/report', { id, type, commentID })
      .catch((err) => {
        console.log(err);
      });
    let vid = { ...currentVid };
